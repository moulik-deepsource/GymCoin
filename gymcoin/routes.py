...
def home():
    ...
    return render_template(
        "blockchain.html", title="Blockchain", blockchain=blockchainObj
    )
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash(
            f"Account created for @{form.username.data}! You are now logged in as well.",
            "success",
        )
...
def mine():
    ...
    if len(blockchainObj.pendingTransactions) <= 2:
        flash(f"Not enough pending transactions to mine! (Must be > 2)", "danger")
    else:
        feedback = blockchainObj.minePendingTransactions(miner)
        if feedback:
            flash(
                f"Block Mined! Your mining reward has now been added to the pending transactions!",
                "success",
            )
        else:
            flash(f"Error!", "danger")
